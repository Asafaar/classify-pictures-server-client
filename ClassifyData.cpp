...
void Command::execute() {
    if (data->classifiedFile->empty() || data->unclassifiedFile->empty()) {
        this->write("Upload the data please");
        return;
        else {
            InputFile inputFile;
            auto *classifiedVector = new std::vector<VectorData *>;
            auto vectorToClassify = new std::vector<vector<int> *>;
            ...
            while (std::getline(buffer, token, '\n')) {
                ...
                auto *singleUnclassifiedVector = new std::vector<int>;
            ...
            for (vector<int> *vec: *vectorToClassify) {
